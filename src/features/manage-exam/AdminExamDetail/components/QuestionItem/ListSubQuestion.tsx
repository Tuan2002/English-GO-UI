...
const ListSubQuestion = ({ question, questionResult }: { question: IExamQuestion; questionResult?: IResultOfQuestion }) => {
  const { targetQuestionOfSkill, currentTargetQuestion } = useSelector((state: RootState) => state.examStore);
  useEffect(() => {
    if (currentTargetQuestion?.questionId) {
  ...
  }, [currentTargetQuestion?.questionId]);
  return (
...
export default ListSubQuestion;
